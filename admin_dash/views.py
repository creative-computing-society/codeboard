        ...
        if request.method == "POST":
            ...
            if user_obj and user_obj.is_staff:
                login(
                    request,
                    user_obj,
                    backend="ccs_auth.auth_backends.SSOAuthenticationBackend",
                )
                return redirect("admin_dash:admin_dashboard")
...
def edit_question(request, question_id, *args, **kwargs):
    ...
    if request.method == "POST":
        ...
        datetime_object = datetime_object.replace(tzinfo=IST)
...
def add_ques_from_id(request):
    ...
    if request.method == "POST":
        ...
        count = Question.objects.filter(pk__in=ques_ids).count()

        if count == len(ques_ids):
...
def delete_ques_with_ids(request):
    ...
    try:
        ques_ids = request.POST.get("selected_questions")
        if not ques_ids:
            return redirect("admin_dash:admin_dashboard")

        ques_ids_list = [int(x) for x in ques_ids.split(",")]

        logger.debug(f"Deleting questions with IDs: {ques_ids_list}")

        deleted_count, _ = Question.objects.filter(pk__in=ques_ids_list).delete()

        if deleted_count == len(ques_ids_list):
            logger.info(f"All questions deleted successfully: {ques_ids_list}")
        else:
            logger.warning(
                f"Only {deleted_count}/{len(ques_ids_list)} questions were deleted."
            )
...
def validate_ques_ids(ques_ids):
    try:
        ...
        existing_ids = set(
            Question.objects.filter(pk__in=id_list).values_list("pk", flat=True)
        )
        ...
        logger.error("Invalid input: must be comma-separated integers.")
        return []
