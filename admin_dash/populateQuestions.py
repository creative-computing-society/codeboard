...
from leaderboard.query_manager import fetch_all_questions
...
def populate_question_model(frontendQuestionId):
    try:
        logger.info(
            f"Populating question model for frontendQuestionId {frontendQuestionId}..."
        )

        questions_dict = async_to_sync(fetch_all_questions)()
        ...
        if question_data:
            ...
            if created:
                logger.info(
                    f"Question with frontendQuestionId {frontendQuestionId} created successfully."
                )
                obj.questionDate = timezone.now()
                obj.save()
            else:
                logger.info(
                    f"Question with frontendQuestionId {frontendQuestionId} updated successfully."
                )
        else:
            logger.warning(
                f"Question with frontendQuestionId {frontendQuestionId} not found."
            )

    except Exception as e:
        logger.error(
            f"Error populating question model for frontendQuestionId {frontendQuestionId}: {e}"
        )
